...
//-- Martin Webrant
          ...
          if (g_IsSpectator[m_iSortedRows[row]])
            ...
            sprintf(sz, "%s  ", AgGetRealName(m_iSortedRows[row]).c_str());

          /*
          if (g_IsSpectator[m_iSortedRows[row]])
					  sprintf(sz, "%s  (spectator)", pl_info->name);
          else
            sprintf(sz, "%s  ", pl_info->name);
            */
          AgStripColors(sz);
          ...
          */
          //-- Martin Webrant
				...
				case COLUMN_TRACKER:
		      if (g_iPlayerFlag1 == m_iSortedRows[row] || g_iPlayerFlag2 == m_iSortedRows[row])
            pLabel->setImage(m_pCTFScoreboard);
          else
            pLabel->setImage(NULL);
					break;
				case COLUMN_KILLS:
