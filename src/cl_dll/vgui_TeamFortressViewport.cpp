...
//++ BulliT
	...
	{
		pButton = new ClassButton(0, pButtonText, 0, BUTTON_SIZE_Y * m_pCurrentCommandMenu->GetNumButtons(), CMENU_SIZE_X, BUTTON_SIZE_Y, false);
		pButton->addActionSignal(new CMenuHandler_TextWindow(MENU_README));
		// Create an input signal that'll popup the current menu
		pButton->addInputSignal( new CMenuHandler_PopupSubMenuInput(pButton, m_pCurrentCommandMenu) );
	}
//-- Martin Webrant
...
void TeamFortressViewport::ToggleIRC()
{
	...
	if ( m_pIRC->isVisible() )
	{
		m_pIRC->setVisible( false );
	}
	else
	{
		m_pIRC->setVisible( true );
	}

	UpdateCursorState();
}

void TeamFortressViewport::ToggleWinamp()
{
	if (!m_iInitialized)
		return;
	...
	if ( m_pWinamp->isVisible() )
	{
		m_pWinamp->setVisible( false );
	}
	else
	{
		m_pWinamp->setVisible( true );
	}

	UpdateCursorState();
}

void TeamFortressViewport::UserCmd_Winamp()
{
	if (!m_iInitialized)
		return;
  ...
  m_pWinamp->UserCmd_Winamp();
}


void TeamFortressViewport::ShowPassword()
{
	if (!m_iInitialized)
		return;
...
void TeamFortressViewport::HidePassword()
{
	if (!m_iInitialized)
		return;
...
void TeamFortressViewport::ToggleMapBrowser()
{
	if (!m_iInitialized)
		return;
	...
	if ( m_pMapBrowser->isVisible() )
	{
		m_pMapBrowser->setVisible( false );
	}
	else
	{
    m_pMapBrowser->GetMaps();
		m_pMapBrowser->setVisible( true );
	}
...
#define IRC_WINDOW_X				XRES(112)
#define IRC_WINDOW_Y				YRES(80)
#define IRC_WINDOW_SIZE_X			XRES(424)
#define IRC_WINDOW_SIZE_Y			YRES(312)

#define PASSWORD_WINDOW_X				XRES(112)
#define PASSWORD_WINDOW_Y				YRES(80)
#define PASSWORD_WINDOW_SIZE_X	XRES(400)
#define PASSWORD_WINDOW_SIZE_Y	YRES(200)

#define MAPBROWSER_INDENT_X			XRES(90)
#define MAPBROWSER_INDENT_Y			YRES(40)

void TeamFortressViewport::CreateIRC( void )
{
	m_pIRC = new AGVGuiIRC(IRC_INDENT_X, IRC_INDENT_Y, ScreenWidth - (IRC_INDENT_X * 2), ScreenHeight - (IRC_INDENT_Y * 2));
	//m_pIRC = new AGVGuiIRC(IRC_WINDOW_X, IRC_WINDOW_Y, IRC_WINDOW_SIZE_X, IRC_WINDOW_SIZE_Y);
//	m_pIRC = new AGVGuiIRC( 0, 0, ScreenWidth/2, ScreenHeight/2 );
	m_pIRC->setParent(this);
	m_pIRC->setVisible(false);
}

void TeamFortressViewport::CreateWinamp( void )
{
  m_pWinamp = new AGVGuiWinamp(IRC_WINDOW_X, IRC_WINDOW_Y, IRC_WINDOW_SIZE_X, IRC_WINDOW_SIZE_Y);
//	m_pWinamp = new AGVGuiWinamp( 0, 0, ScreenWidth/2, ScreenHeight/2 );
	m_pWinamp->setParent(this);
	m_pWinamp->setVisible(false);
}

void TeamFortressViewport::CreatePassword( void )
{
	m_pPassword = new AgVGuiPassword(PASSWORD_WINDOW_X, PASSWORD_WINDOW_Y, PASSWORD_WINDOW_SIZE_X, PASSWORD_WINDOW_SIZE_Y);
	m_pPassword->setParent(this);
	m_pPassword->setVisible(false);
}

void TeamFortressViewport::CreateMapBrowser( void )
{
	m_pMapBrowser = new AgVGuiMapBrowser(MAPBROWSER_INDENT_X, MAPBROWSER_INDENT_Y, ScreenWidth - (MAPBROWSER_INDENT_X * 2), ScreenHeight - (MAPBROWSER_INDENT_Y * 2));
	m_pMapBrowser->setParent(this);
	m_pMapBrowser->setVisible(false);
}
...
//++ BulliT
  ...
  if (m_pPassword)
    if (0 == m_pPassword->KeyInput(down,keynum,pszCurrentBinding))
      return 0;

  if (m_pMapBrowser)
    if (0 == m_pMapBrowser->KeyInput(down,keynum,pszCurrentBinding))
      return 0;

  if (m_pServerBrowser)
    if (0 == m_pServerBrowser->KeyInput(down,keynum,pszCurrentBinding))
      return 0;
